...
path = ""
asso = pd.read_csv(path + asso_file, sep="\t")
...
for c in inpf:
    if c.startswith("/"):
        ...
        record = asso[(asso['TagID'] == col) & (asso.ExperimentID.str.contains(asso_col))].iloc[0]
        colf = record['CompleteAmplificationID']
        colf_af1 = record['AddedField1']
        # colf_af1 = record['AddedField8_FacsSeparationPurity']
...
df['delta'] = df.apply(
    lambda x: x['target_end_r2'] - x['target_end'] if x['target_strand'] == '+' else x['target_start'] - x[
        'target_start_r2'], axis=1)
...
path = ""
#df = pd.read_csv(path + "results_FERRARI_AAV.v3.tsv", sep="\t")
df = df[df.aav_matches>=30]

dfg = pd.DataFrame({'count': df.groupby(
    ["target_chr", "integration_locus", "target_strand", "junction_locus", "aav_last_strand", "gap", "input_file",
     "partial_id", "aav_alignments_start_end", "n_aav_aln"]).size()}).reset_index()

dfg = pd.DataFrame({'count': dfg.groupby(
    ["target_chr", "integration_locus", "target_strand", "junction_locus", "aav_last_strand", "gap", "partial_id",
     "aav_alignments_start_end", "n_aav_aln"])['count'].sum()}).reset_index()
# dfg = pd.DataFrame({'count': dfg.groupby(
...
for index, row in dfg.iterrows():
    ...
    if k not in results.keys():
        ...
        results[k][row.junction_locus][row.partial_id] = row['count']
    else:
        junction_present = False
        for other_junction in results[k]:
            if abs(other_junction - row.junction_locus) == 0:
                junction_present = True
                if row.partial_id in results[k][other_junction].keys():
                    results[k][other_junction][row.partial_id] += row['count']
                else:
                    results[k][other_junction][row.partial_id] = row['count']
                break
...
r_orig = r[old_df_col]
r_new = r[key_cols]
r_new.to_csv(path + outputbn+"_SeqCount_gap_junction_complete.minfvalue30.aggregAF1.tsv", sep="\t", index=False)
df_seq_count = r_new.copy()

dfg = pd.DataFrame({'count': df.groupby(
    ["target_chr", "integration_locus", "target_strand", "junction_locus", "gap", "input_file", "partial_id",
     "aav_alignments_start_end", "n_aav_aln", "aav_last_strand", "total_matches"
     ]).size()}).reset_index()
dfg = pd.DataFrame({'count': dfg.groupby(
    ["target_chr", "integration_locus", "target_strand", "junction_locus", "gap", "input_file", "partial_id",
     "aav_alignments_start_end", "n_aav_aln", "aav_last_strand"]).size()}).reset_index()

dfg = pd.DataFrame({'count': dfg.groupby(
    ["target_chr", "integration_locus", "target_strand", "junction_locus", "gap", "partial_id",
     "aav_alignments_start_end", "n_aav_aln", "aav_last_strand"])['count'].sum()}).reset_index()
# dfg = pd.DataFrame({'count': dfg.groupby(
#    ["target_chr", "integration_locus","target_strand", "junction_locus", "gap","input_file"]).size()}).reset_index()
# dfg['junction_locus']=dfg['junction_locus'].map(lambda x: itr3_end-(x-itr5_start) if itr5_start<=x<=itr5_end else x)
...
r_orig = r[old_df_col]
r_new = r[key_cols]
r_new.to_csv(path + outputbn+"_ShsCount_gap_junction_complete.minfvalue30.aggregAF1.tsv", sep="\t", index=False)
                ...
                if tag in gs_cols:
                    ...
                    if df_group_sorted.shape[0]>1 and not np.isnan(df_group_sorted.iloc[1][tag]) and df_group_sorted.iloc[0][tag] == \
                        ...
                        df_group_seq_count_sorted = df_group_seq_count.sort_values(by=[tag],
                                                                                   ...
                                                                                   na_position='last')
                        if (key_of_gp != df_group_seq_count_sorted.index[0]):
                            print(tag,key_of_gp, df_group_sorted.loc[key_of_gp, tag], df_group_seq_count_sorted.index[0],
                                  df_group_seq_count_sorted.loc[df_group_seq_count_sorted.index[0], tag])
                        key_of_gp = df_group_seq_count_sorted.index[0]
...
r['IS_genomicID'] = r.apply(
    lambda x: "_".join([x['integration_chr'], str(x['integration_locus']), x['integration_strand']]), axis=1)
...
cdf.to_csv(path + outputbn+"_ShsCount_gap_junction_complete.v2.minfvalue30.maxgapabs20.aggregAF1.window8_10.tsv",
           sep="\t", index=False)
...
cdf_cleaned.to_csv(
    path +  outputbn+"_ShsCount_gap_junction_complete.v2.minfvalue30.maxgapabs20.aggregAF1.window8_10.cleaned1.tsv",
    sep="\t", index=False)
...
cdf.to_csv(path + outputbn+"_SeqCount_gap_junction_complete.v2.minfvalue30.maxgapabs20.aggregAF1.window8_10.tsv",
           sep="\t", index=False)
cols_sum = cdf.columns[4:]
cdf['sum'] = cdf[cols_sum].sum(axis=1)
cdf_cleaned = cdf[cdf['sum'] > 1].copy()
cdf_cleaned = cdf_cleaned.drop('sum', axis=1)
cdf_cleaned.to_csv(
    path + outputbn+ "_SeqCount_gap_junction_complete.v2.minfvalue30.maxgapabs20.aggregAF1.window8_10.cleaned1.tsv",
    sep="\t", index=False)
